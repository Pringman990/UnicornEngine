...
namespace TGA
{
	namespace FBX
	{
		class Internals
		{
			...
			static bool GatherSkeleton(FbxNode* aRootNode, FbxImportStatus& inoutStatus, Skeleton& inOutSkeleton, std::vector<Skeleton::Bone>& outEventBones, std::vector<Skeleton::Socket>&
			                           outSockets, int someParentIndex = -1);

			static bool FbxMeshToMesh(FbxScene* aScene, FbxNode* aMeshNode, Skeleton& aSkeleton, FbxImportStatus& inoutStatus, std::vector<Mesh::Element>& outMeshes, bool bMergeDuplicateVertices);
