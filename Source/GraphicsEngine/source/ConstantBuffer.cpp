...
bool ConstantBuffer::Init(uint32_t aSize, void* someData)
{
	...
	HRESULT result = Engine::GetGraphicsEngine().GetDX11()->GetDevice()->CreateBuffer(&desc, nullptr, &mBuffer);
	if (FAILED(result))
	{
		_com_error err(result);
		LPCTSTR errorMessage = err.ErrorMessage();
		std::wcout << "Failed to create constant buffer: " << errorMessage << std::endl;
		return false;
	}
	return true;
}
