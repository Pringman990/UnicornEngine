...
{
    HRESULT result = DirectX::CreateDDSTextureFromFile(Engine::GetGraphicsEngine().GetDX11()->GetDevice(), aPath, nullptr, mSRV.GetAddressOf());
    if (FAILED(result))
    {
        result = DirectX::CreateDDSTextureFromFile(Engine::GetGraphicsEngine().GetDX11()->GetDevice(), mDefaultMaterialTexturePath.c_str(), nullptr, mSRV.GetAddressOf());
...
{
    Engine::GetGraphicsEngine().GetDX11()->GetDeviceContext()->PSSetShaderResources(aSlot, 1, mSRV.GetAddressOf());
}
