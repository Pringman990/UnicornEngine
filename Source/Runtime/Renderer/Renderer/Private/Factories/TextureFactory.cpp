...
GPUResourceHandle<GPUTexture> TextureFactory::CreateTextureRenderTarget(LogicalDevice& Device, VkExtent2D Extent, VkFormat Format)
{
	GPUResourceHandle<GPUTexture> handle = Renderer::Get()->GetGPUResourceManager().AllocateResource<GPUTexture>();
	GPUTexture* texture = Renderer::Get()->GetGPUResourceManager().GetResource(handle);
	
	VkExtent3D extent = { Extent.width, Extent.height, 1 };
	CreateTexture(texture, extent, Format, VK_IMAGE_USAGE_SAMPLED_BIT | VK_IMAGE_USAGE_TRANSFER_DST_BIT | VK_IMAGE_USAGE_STORAGE_BIT);
