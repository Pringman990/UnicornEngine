...
GPUResourceHandle<GPUTexture> GPUTextureManager::CreateTexture2D(const Vector2i& Extent, RenderFormat Format, TextureBindFlags Bind, TextureUsage Usage)
{
	...
	gpuTexture.binds = Bind;

	return mTexturePool.Allocate(gpuTexture);
}

GPUResourceHandle<GPUTexture> GPUTextureManager::CreateTexture(const ByteBuffer& Buffer, const Vector3i& Extent, RenderFormat Format, TextureBindFlags Bind, TextureUsage Usage)
{
	const LogicalDevice& device = mRenderer->GetLogicalDevice();
	...
	subResource.SysMemPitch = Extent.x * 4;
	subResource.SysMemSlicePitch = Extent.x * Extent.y * 4;
	HRESULT hr = device->CreateTexture2D(&desc, &subResource, gpuTexture.texture.GetAddressOf());
	if (FAILED(hr))
	{
		LOG_ERROR("Failed to create Texture2D (Format={}): {}", (int)dxFormat, hr);
		return GPUResourceHandle<GPUTexture>::Invalid();
...
GPUResourceHandle<GPUTexture> GPUTextureManager::CreateTextureRenderTarget(ID3D11Texture2D* Texture)
{
	const LogicalDevice& device = mRenderer->GetLogicalDevice();
	...
	gpuTexture.binds |= TextureBindFlags::RenderTarget;

    return mTexturePool.Allocate(gpuTexture);
}

void GPUTextureManager::CreateTextureView(GPUTexture* Texture)
{
	const LogicalDevice& device = mRenderer->GetLogicalDevice();
	...
	Texture->binds |= TextureBindFlags::ShaderRead;
}

void GPUTextureManager::ResizeTexture2D(GPUResourceHandle<GPUTexture> Handle, const Vector2i& NewExtent)
{
	GPUTexture* gpuTex = mTexturePool.Get(Handle);
	if(!gpuTex)
	{
		LOG_ERROR("Failed to resize texture2d because handle returned nullptr");
		return;
	}

	D3D11_TEXTURE2D_DESC oldTextureDesc = {};
	gpuTex->texture->GetDesc(&oldTextureDesc);

	oldTextureDesc.Width = NewExtent.x;
	oldTextureDesc.Height = NewExtent.y;

	D3D11_SHADER_RESOURCE_VIEW_DESC oldSrvDesc = {};
	gpuTex->srv->GetDesc(&oldSrvDesc);

	D3D11_DEPTH_STENCIL_VIEW_DESC oldDsvDesc = {};
	gpuTex->dsv->GetDesc(&oldDsvDesc);

	D3D11_RENDER_TARGET_VIEW_DESC oldRtvDesc = {};
	gpuTex->rtv->GetDesc(&oldRtvDesc);

	TextureBindFlags binds = gpuTex->binds;
	RenderFormat format = gpuTex->format;

	GPUTexture::Reset(gpuTex);

	const LogicalDevice& device = mRenderer->GetLogicalDevice();
	DXGI_FORMAT dxFormat = ToDXFormat(format);
	HRESULT hr = device->CreateTexture2D(&oldTextureDesc, nullptr, gpuTex->texture.GetAddressOf());
	if (FAILED(hr))
	{
		LOG_ERROR("Failed to create Texture2D (Format={}): {}", (int)dxFormat, hr);
		return;
	...
	// RTV (Color)
	if (HasFlag(binds, TextureBindFlags::RenderTarget))
	{
		hr = device->CreateRenderTargetView(gpuTex->texture.Get(), &oldRtvDesc, gpuTex->rtv.GetAddressOf());
		if (FAILED(hr))
		{
			LOG_ERROR("Failed to create RTV: {}", hr);
			return ;
	...
	// DSV (Depth)
	if (HasFlag(binds, TextureBindFlags::DepthStencil))
	{
		hr = device->CreateDepthStencilView(gpuTex->texture.Get(), &oldDsvDesc, gpuTex->dsv.GetAddressOf());
		if (FAILED(hr))
		{
			LOG_ERROR("Failed to create DSV: {}", hr);
			return;
	...
	// SRV (Shader read)
	if (HasFlag(binds, TextureBindFlags::ShaderRead))
	{
		hr = device->CreateShaderResourceView(gpuTex->texture.Get(), &oldSrvDesc, gpuTex->srv.GetAddressOf());
		if (FAILED(hr))
		{
			LOG_ERROR("Failed to create SRV: {}", hr);
			return;
	...
	// Viewport setup
	gpuTex->viewport.TopLeftX = 0.0f;
	gpuTex->viewport.TopLeftY = 0.0f;
	gpuTex->viewport.Width = (float)NewExtent.x;
	gpuTex->viewport.Height = (float)NewExtent.y;
	gpuTex->viewport.MinDepth = 0.0f;
	gpuTex->viewport.MaxDepth = 1.0f;
