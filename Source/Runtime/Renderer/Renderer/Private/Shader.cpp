...
std::shared_ptr<Shader> Shader::CreateDefaultVoxel()
{
	...
	std::string vsData;
	{
		//Load Vertex Shader
		std::ifstream vsFile;
		vsFile.open("../../Binaries/Shaders/Mesh_VS.cso", std::ios::binary);
		...
		//Load Pixel Shader
		std::ifstream psFile;
		psFile.open("../../Binaries/Shaders/Ray_March_GBuffer_PS.cso", std::ios::binary);
...
std::shared_ptr<Shader> Shader::CreateDefaultChunk()
{
	Renderer* renderer = Renderer::GetInstance();
	ID3D11Device* device = renderer->GetDevice();
	...
	std::string vsData;
	{
		...
		//Load Pixel Shader
		std::ifstream psFile;
		psFile.open("../../Binaries/Shaders/Chunk_PS.cso", std::ios::binary);
		std::string psData = { std::istreambuf_iterator<char>(psFile), std::istreambuf_iterator<char>() };
		result = device->CreatePixelShader(psData.data(), psData.size(), nullptr, &shader->mPixelShader);
		if (FAILED(result))
		{
...
std::shared_ptr<Shader> Shader::CreateDefaultPolygon()
{
	Renderer* renderer = Renderer::GetInstance();
	ID3D11Device* device = renderer->GetDevice();
	...
	std::string vsData;
	{
		...
		//Load Pixel Shader
		std::ifstream psFile;
		psFile.open("../../Binaries/Shaders/Single_Color_Mesh_PS.cso", std::ios::binary);
		std::string psData = { std::istreambuf_iterator<char>(psFile), std::istreambuf_iterator<char>() };
		result = device->CreatePixelShader(psData.data(), psData.size(), nullptr, &shader->mPixelShader);
		if (FAILED(result))
		{
