...
namespace TGA
{
	namespace FBX
	{
		...
		bool Internals::GatherSkeleton(FbxNode* aRootNode, FbxImportStatus& inoutStatus, Skeleton& inOutSkeleton, std::vector<Skeleton::Bone>& outEventBones, std::vector<Skeleton::Socket>& outSockets, int someParentIndex)
		{
				...
				case FbxNodeAttribute::eSkeleton:
					{
						...
						{
							inoutStatus.AddWarning(FbxImportWarning::DuplicateBoneNames);
						}
			...
			for (int i = 0; i < aRootNode->GetChildCount(); i++)
			{
				GatherSkeleton(aRootNode->GetChild(i), inoutStatus, inOutSkeleton, outEventBones, outSockets, myIndex);
			}
		...
		bool Internals::FbxMeshToMesh(FbxScene* aScene, FbxNode* aMeshNode, Skeleton& aSkeleton, FbxImportStatus& inoutStatus, 
		                              std::vector<Mesh::Element>& outMeshes, bool bMergeDuplicateVertices)
		{
			...
			const bool hasNormals = fbxMesh->GetElementNormalCount() > 0;
			const bool hasTangents = fbxMesh->GetElementTangentCount() > 0;
			const bool hasBinormals = fbxMesh->GetElementBinormalCount() > 0;
				...
				for (int p = 0; p < fbxMeshPolygonCount; p++)
				{
					...
					for (int v = 0; v < 3; v++)
					{
						...
						for (int col = 0; col < fbxNumVxColorChannels && col < 4; col++)
						{
							...
							case FbxGeometryElement::eByControlPoint:
							{
								// DB 231208 Why was this fbxNormalElement?
						...
						if (aSkeleton.GetRoot())
						{
							...
							struct VxBoneWeights
							{
								unsigned BoneID;
								float Weight;
							};
							std::vector<VxBoneWeights> weights;

							int idx = 0;
							for (MMIter it = values.first; it != values.second; ++it)
							{
								std::pair<size_t, float> BoneAndWeight = it->second;
								VxBoneWeights weight;
								weight.BoneID = static_cast<unsigned>(BoneAndWeight.first);
								weight.Weight = BoneAndWeight.second;
								weights.emplace_back(weight);
								idx++;
							}

							std::sort(weights.begin(), weights.end(), [](const VxBoneWeights& A, const VxBoneWeights& B)
							{
							...
							{
								VxBoneWeights& last = weights.back();
								extraWeight += last.Weight;
								weights.pop_back();
							}
							
							if(extraWeight > 0)
							{
								float dividend = extraWeight / 4;
								weights[0].Weight += dividend;
								weights[1].Weight += dividend;
								weights[2].Weight += dividend;
								weights[3].Weight += dividend;

								vxHasExtraWeights = true;
							}

							for(size_t w = 0; w < weights.size(); w++)
							{
								BoneIDs[w] = weights[w].BoneID;
								BoneWeights[w] = weights[w].Weight;
							}
						...
						if(bMergeDuplicateVertices)
						{
							...
							if (VertexDuplicateAccelMap.find(hash) == VertexDuplicateAccelMap.end())
							{
								...
								if(vxHasExtraWeights)
								{
									inoutStatus.AddWarning(FbxImportWarning::TooManySkinWeights);
								}
							}
							else
							{
