...
#ifndef DegToRad
#define DegToRad ((float)(3.14159265358979323846/180.0))
#endif

#ifndef RadToDeg
#define RadToDeg ((float)(180.0/3.14159265358979323846))
#endif
...
inline DirectX::XMMATRIX CreateTRSMatrix(
    DirectX::XMFLOAT3 t,          // Translation values
    DirectX::XMFLOAT3 r,          // Rotation values (in radians)
    DirectX::XMFLOAT3 s           // Scale values
) {
    // Create individual transformation matrices
    DirectX::XMMATRIX matTranslate = DirectX::XMMatrixTranslation(t.x, t.y, t.z);
    DirectX::XMMATRIX matRotate = DirectX::XMMatrixRotationRollPitchYaw(r.x * DegToRad, r.y * DegToRad, r.z * DegToRad);
    DirectX::XMMATRIX matScale = DirectX::XMMatrixScaling(s.x, s.y, s.z);

    // Combine transformations. The order matters: Scale -> Rotate -> Translate.
    return matScale * matRotate * matTranslate;

	/*return DirectX::XMMatrixTranslation(t.x, t.y, t.z)
        * DirectX::XMMatrixRotationRollPitchYaw(r.x * DegToRad, r.y * DegToRad, r.z * DegToRad)
        * DirectX::XMMatrixScaling(s.x, s.y, s.z);*/
}

inline DirectX::XMMATRIX CreateSQTMatrix
(
...
{
    DirectX::XMMATRIX matScale = DirectX::XMMatrixScalingFromVector ( scale );
    DirectX::XMMATRIX matQuaternion = DirectX::XMMatrixRotationQuaternion ( quaternion );
    DirectX::XMMATRIX matTranslate = DirectX::XMMatrixTranslationFromVector ( position );

    return matScale * matQuaternion * matTranslate;
}
inline DirectX::XMMATRIX CreateSQTMatrixEuler
(
	DirectX::XMVECTOR scale,
	DirectX::XMVECTOR rotation, // in radians
	DirectX::XMVECTOR position
)
{
	DirectX::XMMATRIX matScale = DirectX::XMMatrixScalingFromVector(scale);
	DirectX::XMMATRIX matRotation = DirectX::XMMatrixRotationRollPitchYawFromVector(rotation);
	DirectX::XMMATRIX matTranslate = DirectX::XMMatrixTranslationFromVector(position);

	return matScale * matRotation * matTranslate;
}

inline float RoundToDecimalFloat(float num, int dec)
{
	float mult = pow(10.0f, (float)dec);
...
namespace DirectX
{
    namespace SimpleMath
    {
        ...
        struct Vector3 : public XMFLOAT3
        {
            ...
            Vector3(const Vector2& V, float iz) noexcept : XMFLOAT3(V.x, V.y, iz) { }
            ...
            Vector2 ToVec2() const noexcept { return Vector2(x, y); }
        ...
        struct Vector4 : public XMFLOAT4
        {
            ...
            Vector4(const Vector3& V, const float& W) noexcept : XMFLOAT4(V.x, V.y, V.z, W) {}
        ...
        Vector4 operator* (const Matrix& M, const Vector4& V1) noexcept;
        Vector4 operator/ (const Vector4& V1, const Vector4& V2) noexcept;
        Vector4 operator/ (const Vector4& V, float S) noexcept;
        ...
        struct Matrix : public XMFLOAT4X4
        {
            ...
            inline static Matrix RoundToDecimal(Matrix matrix, int decimal) 
            {

				for (int i = 0; i < 16; i++)
				{
					float* ghetto = ((float*)&matrix)+i;
                    *ghetto = RoundToDecimalFloat(*ghetto, decimal);
				}

				return matrix;
            }

            inline static void Print(Matrix matrix)
            {
                /*std::cout << "\t" << matrix.m[0][0] << "\t" << matrix.m[0][1] << "\t" << matrix.m[0][2] << "\t" << matrix.m[0][3] << "\n";
                std::cout << "\t" << matrix.m[1][0] << "\t" << matrix.m[1][1] << "\t" << matrix.m[1][2] << "\t" << matrix.m[1][3] << "\n";
                std::cout << "\t" << matrix.m[2][0] << "\t" << matrix.m[2][1] << "\t" << matrix.m[2][2] << "\t" << matrix.m[2][3] << "\n";
                std::cout << "\t" << matrix.m[3][0] << "\t" << matrix.m[3][1] << "\t" << matrix.m[3][2] << "\t" << matrix.m[3][3] << "\n";*/

                std::cout << RoundToDecimalFloat(matrix.m[0][0], 3) << "\t" << RoundToDecimalFloat(matrix.m[0][1], 3) << "\t" << RoundToDecimalFloat(matrix.m[0][2], 3) << "\t" << RoundToDecimalFloat(matrix.m[0][3], 3) << "\n";
                std::cout << RoundToDecimalFloat(matrix.m[1][0], 3) << "\t" << RoundToDecimalFloat(matrix.m[1][1], 3) << "\t" << RoundToDecimalFloat(matrix.m[1][2], 3) << "\t" << RoundToDecimalFloat(matrix.m[1][3], 3) << "\n";
                std::cout << RoundToDecimalFloat(matrix.m[2][0], 3) << "\t" << RoundToDecimalFloat(matrix.m[2][1], 3) << "\t" << RoundToDecimalFloat(matrix.m[2][2], 3) << "\t" << RoundToDecimalFloat(matrix.m[2][3], 3) << "\n";
                std::cout << RoundToDecimalFloat(matrix.m[3][0], 3) << "\t" << RoundToDecimalFloat(matrix.m[3][1], 3) << "\t" << RoundToDecimalFloat(matrix.m[3][2], 3) << "\t" << RoundToDecimalFloat(matrix.m[3][3], 3) << "\n";
            }

            inline Vector4 Row(int index) const noexcept
            {
    ...
    #include "SimpleMath.inl"
        ...
        static float Distance(Vector3 a, Vector3 b)
        {
            ...
            float num2 = a.y - b.y;
            float num3 = a.z - b.z;
            return std::sqrtf(num * num + num2 * num2 + num3 * num3);
        }

        static inline bool CheckAproxEQ(Vector3 firstPos, Vector3 secPos)
        {
            return Distance(firstPos, secPos) < 0.01f;
        }
